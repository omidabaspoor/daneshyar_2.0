<?php
/**
 * دانش‌یار - ایجنت برنامه‌ریز درسی
 */
require_once __DIR__ . '/../includes/functions.php';

$user = current_user();
if (!$user) {
    redirect(BASE_URL . '/login.php?next=' . urlencode(BASE_URL . '/agents/planner.php'));
}
if (is_banned($user)) { redirect(BASE_URL . '/logout.php'); }

$page = 'agents';
$pageTitle = 'برنامه‌ریز درسی';
$seoTitle = 'برنامه‌ریز درسی با هوش مصنوعی | دانش‌یار';
$seoDescription = 'برنامهٔ درسی روزانه بر اساس پایه، رشته و تاریخ امتحان.';
...
<div class="agent-page">
  <div class="agent-nav-bar">
    ...
    <span class="agent-nav-current">برنامه‌ریز درسی</span>
  </div>

  <section class="agent-hero" data-agent="planner">
    <div class="agent-hero-row">
      <div class="agent-hero-ico"><?= icon('clock') ?></div>
      <div>
        <h1 class="agent-hero-title">برنامه‌ریز درسی</h1>
        <p class="agent-hero-sub">تاریخ امتحان و ساعت مطالعه‌ات رو بگو. یه برنامهٔ واقع‌بینانه برات می‌چینم که هم پر بازده باشه، هم خسته‌کننده نباشه.</p>
      </div>
    </div>
  </section>
  ...
  <form class="agent-panel" id="planForm">
    <div class="agent-panel-head">
      <div class="agent-panel-title"><?= icon('calendar') ?> مشخصات برنامه</div>
    </div>

    <div class="agent-options">
      <label class="agent-option"><input type="radio" name="subject" value="konkoor" required checked><span>کنکور سراسری</span></label>
    ...
    <div class="agent-options">
      <label class="agent-option"><input type="radio" name="grade" value="10" checked><span>پایه ۱۰</span></label>
      <label class="agent-option"><input type="radio" name="grade" value="11"><span>پایه ۱۱</span></label>
    ...
    <input type="text" name="exam_date" class="agent-textarea" placeholder="تاریخ امتحان (مثلاً: ۱۴۰۵/۰۴/۱۵ یا «۲۰ روز دیگر»)" style="margin-top: 10px; min-height: 50px;" required>

    <textarea class="agent-textarea" name="context" placeholder="اگه نکته خاصی هست (مثلاً «هفتهٔ دیگه امتحان دارم»، «۵ درس باید بخونم»، «ضعیف‌ترم در ریاضی») بنویس." style="margin-top: 10px; min-height: 70px;"></textarea>
  ...
  <div class="agent-result" id="planResult">
    <div class="agent-result-head">
      <?= icon('clock') ?>
      <div class="agent-result-title">برنامهٔ درسی</div>
